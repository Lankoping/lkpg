...
export const Route = createFileRoute('/admin/applications')({
  ...
  loader: async () => {
    const applications = await getFoundaryApplicationsFn()
    return { applications }
  },
...
function ApplicationsPage() {
  ...
  const [activePanel, setActivePanel] = useState<'overview' | 'review'>('overview')
  ...
  const closeApplication = async (applicationId: number) => {
    const confirmed = window.confirm('Close this application? It will be marked rejected and the ticket thread will be closed.')
    if (!confirmed) return
    ...
    try {
      ...
      setActionError(error?.message || 'Could not close application')
    } finally {
          ...
          <div className="grid gap-0 md:grid-cols-[340px_1fr]">
            <aside className="border-b border-border bg-background/50 md:border-b-0 md:border-r">
              <div className="border-b border-border px-4 py-3">
                <p className="text-xs font-medium uppercase tracking-[0.22em] text-primary">Applications</p>
            ...
            {selectedApplication ? (
              <div className="p-5">
                ...
                <div className="mt-4 inline-grid grid-cols-2 gap-2 rounded-xl border border-border bg-background p-1">
                  {(['overview', 'review'] as const).map((panel) => (
                    <button
                      key={panel}
                      type="button"
                      onClick={() => {
                ...
                {activePanel === 'review' && (
                  <div className="mt-4 space-y-3 rounded-xl border border-border bg-background p-4">
                    ...
                    <button
                      ...
                      className="w-full rounded-xl border border-amber-400/30 bg-amber-400/10 px-3 py-2 text-xs text-amber-700 hover:bg-amber-400/20 disabled:opacity-60"
                    >
                      {closingApplicationId === selectedApplication.id ? 'Closing application...' : 'Close application'}
                    </button>
