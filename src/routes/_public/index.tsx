        ...
        <div className="mx-auto max-w-6xl px-4 py-12 md:py-16">
          <div className="mb-8 flex flex-col md:flex-row items-start md:items-end justify-between gap-4">
            <div>
              ...
              <h2 className="font-display text-4xl md:text-5xl tracking-wide text-[#F0E8D8]">Senaste Blogg & Nyhet</h2>
            </div>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <article className="relative group overflow-hidden rounded-sm border border-[#C04A2A]/20 bg-[#141210]/80 p-8 transition-all hover:bg-[#1A1816] hover:border-[#C04A2A]/50">
              ...
              <p className="text-[10px] uppercase tracking-[0.28em] text-[#C04A2A] font-medium mb-4">Blogg</p>
              {latestBlog ? (
                <>
                  ...
                  <p className="text-[#F0E8D8]/80 text-sm leading-relaxed mb-8">{latestBlog.excerpt || 'Läs senaste blogginlägget.'}</p>
                  <a className="inline-flex items-center text-[11px] uppercase tracking-[0.1em] text-[#C04A2A] font-medium hover:text-[#F0E8D8] transition-colors" href={`/blogs/${latestBlog.slug}`}>
                    Läs blogginlägget <span className="ml-2">→</span>
                  </a>
                </>
              ) : (
                <p className="text-[#F0E8D8]/50 text-sm">Ingen blogg publicerad ännu.</p>
              )}
            </article>

            <article className="relative group overflow-hidden rounded-sm border border-[#C04A2A]/20 bg-[#141210]/80 p-8 transition-all hover:bg-[#1A1816] hover:border-[#C04A2A]/50">
              <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-[#C04A2A]/50 to-transparent opacity-0 transition-opacity group-hover:opacity-100" />
              <p className="text-[10px] uppercase tracking-[0.28em] text-[#C04A2A] font-medium mb-4">Nyhet</p>
              {latestNews ? (
                <>
                  ...
                  <p className="text-[#F0E8D8]/80 text-sm leading-relaxed mb-8">{latestNews.excerpt || 'Läs senaste nyheten.'}</p>
                  <a className="inline-flex items-center text-[11px] uppercase tracking-[0.1em] text-[#C04A2A] font-medium hover:text-[#F0E8D8] transition-colors" href={`/nyheter/${latestNews.slug}`}>
                    Läs nyheten <span className="ml-2">→</span>
                  </a>
                </>
              ) : (
                <p className="text-[#F0E8D8]/50 text-sm">Ingen nyhet publicerad ännu.</p>
              )}
            </article>
          </div>
        </div>
